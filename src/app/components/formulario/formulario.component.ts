...
export class FormularioComponent implements OnInit {
    ...
    compararFechas() {
        const fechaInicio = this.fechaInicio.split('-');
        const fechaFin = this.fechaFin.split('-');
        const fechaActual = this.fechaActual.split('-');
        const fechaInicio2 = new Date(fechaInicio[0], (fechaInicio[1] - 1), fechaInicio[2]);
        const fechaFin2 = new Date(fechaFin[0], (fechaFin[1] - 1), fechaFin[2]);
        const fechaActual2 = new Date(fechaActual[0], (fechaActual[1] - 1), fechaActual[2]);
    ...
    habilitarAyuda2() {
        const selectedOrderIds = this.formGroup.value.incapacidadLista
            ...
            .filter(v => v !== null);
        this.discapacidades = '';
        for (let sistema in selectedOrderIds) {
